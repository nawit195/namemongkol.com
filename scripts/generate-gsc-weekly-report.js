...
const PRESETS = {
    ...
    'name-seo': {
        ...
        queryMatchers: [
            ...
            { key: 'birthday-head', pattern: /(ชื่อมงคล|ชื่อเสริมดวง)\s*(คน)?เกิดวัน(อาทิตย์|จันทร์|อังคาร|พุธ|พฤหัสบดี|ศุกร์|เสาร์)(\s*2569)?$/i },
            { key: 'birthday-mid-tail', pattern: /(ชื่อมงคล|ชื่อเสริมดวง).*(คน)?เกิดวัน(อาทิตย์|จันทร์|อังคาร|พุธ|พฤหัสบดี|ศุกร์|เสาร์)/i },
            { key: 'birthday-gender-long-tail', pattern: /(ชื่อ|ตั้งชื่อ).*(ผู้หญิง|ผู้ชาย|ลูกสาว|ลูกชาย).*วัน(อาทิตย์|จันทร์|อังคาร|พุธ|พฤหัสบดี|ศุกร์|เสาร์)/i },
        ...
        cannibalizationPages: [
            ...
            { key: 'birthday-overviews', label: '/names/by-birthday/*', match: (u) => /\/names\/by-birthday\//.test(u) },
            { key: 'birthday-gender-pages', label: '/names/*/by-birthday/*', match: (u) => /\/names\/(boys|girls)\/by-birthday\//.test(u) },
            { key: 'monday-girl-article', label: '/articles/monday-girl-names-*', match: (u) => /\/articles\/monday-girl-names-/.test(u) },
        ],
    },
    'pet-names': {
        title: 'Weekly GSC Report (Pet Names SEO)',
        cohortMatchers: [
            { key: 'pet-name-hub', label: '/pet-name', match: (u) => /\/pet-name$/.test(u) },
            { key: 'dog-names', label: '/pet-name/dog', match: (u) => /\/pet-name\/dog$/.test(u) },
            { key: 'cat-names', label: '/pet-name/cat', match: (u) => /\/pet-name\/cat$/.test(u) },
        ],
        queryMatchers: [
            { key: 'pet-generic', pattern: /(ชื่อสัตว์เลี้ยงมงคล|ตั้งชื่อสัตว์เลี้ยง|ชื่อสัตว์เลี้ยงความหมายดี)/i },
            { key: 'dog-primary', pattern: /(ชื่อหมามงคล|ตั้งชื่อหมา|ชื่อหมาความหมายดี)/i },
            { key: 'cat-primary', pattern: /(ชื่อแมวมงคล|ตั้งชื่อแมว|ชื่อแมวนำโชค)/i },
            { key: 'pet-language', pattern: /(ชื่อหมา|ชื่อแมว).*(ไทย|อังกฤษ|ญี่ปุ่น|เกาหลี)/i },
        ],
        cannibalizationPages: [
            { key: 'pet-name-hub', label: '/pet-name', match: (u) => /\/pet-name$/.test(u) },
            { key: 'dog-names', label: '/pet-name/dog', match: (u) => /\/pet-name\/dog$/.test(u) },
