    ...
    const lockedPanel = queryResult?.canUnlock ? (
        <div ref={lockedRef} className="mt-5 overflow-hidden rounded-xl border border-[#d6c58e] bg-[#0f172a] text-slate-100 shadow-[0_12px_30px_rgba(15,23,42,0.16)]">
            ...
            <div className="p-5 sm:p-6">
                ...
                {error ? <div role="alert" className="mt-4 rounded-lg border border-rose-400/40 bg-rose-400/10 px-4 py-3 text-sm text-rose-100">{error}{error.includes('เครดิตไม่เพียงพอ') ? <Link prefetch={false} href="/topup" className="ml-2 font-bold text-[#f1c75b] underline">เติมเครดิต</Link> : null}</div> : null}
            </div>
        </div>
    ) : null;
    ...
    return (
        <main id="pet-name-tool" className="min-h-screen scroll-mt-20 bg-[#f9f7f2] pb-20 text-[#5a5a82] md:pb-0">
            <section className="relative isolate min-h-[300px] overflow-hidden border-b border-[#e5dfd3] sm:min-h-[340px]">
                <Image src="/images/articles/pet-name-hero.png" alt="สุนัขและแมวสำหรับค้นหาชื่อสัตว์เลี้ยงมงคล" fill priority unoptimized className="object-cover object-center" sizes="100vw" />
