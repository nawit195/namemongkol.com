...
export function generateStaticParams() {
    return BIRTHDAY_DAY_KEYS.map((day) => ({ day }));
}
export async function generateMetadata({ params }: { params: Promise<{ day: string }> }): Promise<Metadata> {
    const { day } = await params;
    if (!isBirthdayDay(day)) return {};
    const copy = getBirthdayPageCopy('boys', day);
    const canonical = `${siteUrl}/names/boys/by-birthday/${day}`;
    return {
...
export default async function BoysBirthdayDayPage({ params }: { params: Promise<{ day: string }> }) {
    const { day } = await params;
    if (!isBirthdayDay(day)) return null;
    return <BirthdayDayLandingPage gender="boys" day={day} />;
}
