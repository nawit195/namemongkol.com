import { useEffect, useState } from "react";
import { Facebook, Youtube, Music2, ChevronLeft, ChevronRight } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogDescription,
} from "@/components/ui/dialog";
import lineQr from "@/assets/line-qr.png";

// LINE doesn't ship in lucide — inline brand glyph
const LineIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" {...props}>
    <path d="M19.365 9.863c.349 0 .63.285.63.631 0 .345-.281.63-.63.63H17.61v1.125h1.755c.349 0 .63.283.63.63 0 .344-.281.629-.63.629h-2.386c-.345 0-.627-.285-.627-.629V8.108c0-.345.282-.63.63-.63h2.386c.346 0 .627.285.627.63 0 .349-.281.63-.63.63H17.61v1.125h1.755zm-3.855 3.016c0 .27-.174.51-.432.596-.064.021-.133.031-.199.031-.211 0-.391-.09-.51-.25l-2.443-3.317v2.94c0 .344-.279.629-.631.629-.346 0-.626-.285-.626-.629V8.108c0-.27.173-.51.43-.595.06-.023.136-.033.194-.033.195 0 .375.104.495.254l2.462 3.33V8.108c0-.345.282-.63.63-.63.345 0 .63.285.63.63v4.771zm-5.741 0c0 .344-.282.629-.631.629-.345 0-.627-.285-.627-.629V8.108c0-.345.282-.63.63-.63.346 0 .628.285.628.63v4.771zm-2.466.629H4.917c-.345 0-.63-.285-.63-.629V8.108c0-.345.285-.63.63-.63.348 0 .63.285.63.63v4.141h1.756c.348 0 .629.283.629.63 0 .344-.282.629-.629.629M24 10.314C24 4.943 18.615.572 12 .572S0 4.943 0 10.314c0 4.811 4.27 8.842 10.035 9.608.391.082.923.258 1.058.59.12.301.079.766.038 1.08l-.164 1.02c-.045.301-.24 1.186 1.049.645 1.291-.539 6.916-4.078 9.436-6.975C23.176 14.393 24 12.458 24 10.314" />
  </svg>
);

const socials = [
  {
    Icon: Facebook,
    href: "https://www.facebook.com/MatrixIntertrade",
    label: "Facebook",
    color: "text-[#1877F2]",
    ring: "hover:ring-[#1877F2]/40",
  },
  {
    Icon: LineIcon,
    href: "https://lin.ee/hMrlrvH",
    label: "LINE",
    color: "text-[#06C755]",
    ring: "hover:ring-[#06C755]/40",
    qr: true,
  },
  {
    Icon: Music2,
    href: "https://www.tiktok.com/@matrixintertrade",
    label: "TikTok",
    color: "text-foreground",
    ring: "hover:ring-foreground/30",
  },
  {
    Icon: Youtube,
    href: "https://www.youtube.com/@matrixintertrade",
    label: "YouTube",
    color: "text-[#FF0000]",
    ring: "hover:ring-[#FF0000]/40",
  },
];

export function FloatingSocial() {
  const [openDesktop, setOpenDesktop] = useState(true);
  const [mounted, setMounted] = useState(false);
  const [lineOpen, setLineOpen] = useState(false);

  useEffect(() => setMounted(true), []);
  if (!mounted) return null;

  return (
    <>
      {/* Desktop: right-side vertical social rail */}
      <div className="hidden md:flex fixed right-0 top-1/2 -translate-y-1/2 z-40 items-center">
        <div
          className={`flex flex-col gap-2 rounded-l-2xl border border-r-0 border-border bg-background/90 backdrop-blur-xl shadow-elev p-2 transition-all duration-300 ${
            openDesktop
              ? "translate-x-0 opacity-100"
              : "translate-x-full opacity-0 pointer-events-none"
          }`}
        >
          {socials.map(({ Icon, href, label, color, ring, qr }) => {
            const cls = `grid h-10 w-10 place-items-center rounded-xl border border-border bg-card transition-all hover:-translate-y-0.5 hover:shadow-card hover:ring-2 ${ring}`;
            if (qr) {
              return (
                <button
                  key={label}
                  type="button"
                  aria-label={label}
                  onClick={() => setLineOpen(true)}
                  className={cls}
                >
                  <Icon className={`h-5 w-5 ${color}`} />
                </button>
              );
            }
            return (
              <a
                key={label}
                href={href}
                target="_blank"
                rel="noopener noreferrer"
                aria-label={label}
                className={cls}
              >
                <Icon className={`h-5 w-5 ${color}`} />
              </a>
            );
          })}
        </div>
        <button
          aria-label={openDesktop ? "ซ่อนโซเชียล" : "แสดงโซเชียล"}
          onClick={() => setOpenDesktop((v) => !v)}
          className="grid h-10 w-6 place-items-center rounded-l-lg border border-r-0 border-border bg-background/90 backdrop-blur-xl shadow-elev text-muted-foreground hover:text-foreground transition-colors -ml-px"
        >
          {openDesktop ? <ChevronRight className="h-4 w-4" /> : <ChevronLeft className="h-4 w-4" />}
        </button>
      </div>

      {/* Mobile: contact actions live in MobileCTABar — avoid duplicate floating rail */}

      <Dialog open={lineOpen} onOpenChange={setLineOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <LineIcon className="h-5 w-5 text-[#06C755]" />
              เพิ่มเพื่อนทาง LINE
            </DialogTitle>
            <DialogDescription>
              สแกน QR Code เพื่อเพิ่ม Matrix Intertrade เป็นเพื่อนใน LINE
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-col items-center gap-4">
            <div className="rounded-2xl border border-border bg-white p-4 shadow-card">
              <img src={lineQr} alt="LINE QR Code" className="h-56 w-56 object-contain" />
            </div>
            <a
              href="https://lin.ee/hMrlrvH"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 rounded-full bg-[#06C755] px-5 py-2.5 text-sm font-bold text-white shadow-card hover:opacity-90"
            >
              <LineIcon className="h-4 w-4" /> เปิดใน LINE
            </a>
          </div>
        </DialogContent>
      </Dialog>
    </>
  );
}
